...
func (u *User) Signout() {
	user := u.SearchUserByName("username", u.Username)
...
func (u *User) SearchUserByName(field, value string) User {
	if field == "username" {
...
func (u *User) SearchUserById(field string, id uint) User {
	if field == "id" {
		db.Where(field+" = ?", id).First(&u)
	}
	return *u
}
